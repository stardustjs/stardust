...
describe("Color", () => {
  it("color", () => {
    ...
    assert.deepEqual(
      c,
      Stardust.Color.FromArray([12 / 255, 34 / 255, 56 / 255, 0.3 * 0.2])
    );
    c = Stardust.Color.FromHTML("rgb(12, 34, 56)");
    assert.deepEqual(
      c,
      Stardust.Color.FromArray([12 / 255, 34 / 255, 56 / 255, 1.0])
    );
    c = Stardust.Color.FromHTML("#1F77b4");
    assert.deepEqual(
      c,
      Stardust.Color.FromArray([0x1f / 255, 0x77 / 255, 0xb4 / 255, 1.0])
    );
    c = Stardust.Color.FromHTML("#efA", 0.3);
    assert.deepEqual(
      c,
      Stardust.Color.FromArray([0xee / 255, 0xff / 255, 0xaa / 255, 0.3])
    );
  });
});
