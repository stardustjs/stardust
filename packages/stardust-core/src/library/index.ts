...
const modules = new Dictionary<Dictionary<SyntaxTree.FileBlockFunction>>();
...
function importPrimitiveCode(name: string, code: string) {
  ...
  const tree = parseFile(code);
  for (const f of tree.blocks) {
    if (f.type == "function") {
      const fn = f as SyntaxTree.FileBlockFunction;
...
export function forEachModuleFunction(
  name: string,
  callback: (fn: SyntaxTree.FileBlockFunction, name: string) => any
) {
