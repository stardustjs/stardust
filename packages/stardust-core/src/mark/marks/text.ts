...
import { BindingValue, Image, Binding, ArrayBinding } from "../../binding";
...
export class TextMark extends Mark {
  ...
  private _textInfo: ArrayBinding;
  ...
  constructor(mode: "2d" | "3d" = "2d", platform: Platform) {
    super(
      compile(`
        mark TextMark(
          ...
          let p = ${
            mode == "2d" ? "Vector3(position.x, position.y, 0)" : "position"
          };
          let info = array(mTextInfo, mIndex);
          let e1 = normalize(cross(${
            mode == "2d" ? "Vector3(up.x, up.y, 0)" : "up"
          }, normal)) * info.x * (scale / mScaling);
    ...
    this._textInfo = new ArrayBinding();
  ...
  public prepare(): Mark {
    if (this._shouldRefreshCanvas) {
      ...
      const texts = new Binding("string", this._textBinding).map(data);
      const fontFamilys = new Binding("string", this._fontFamilyBinding).map(
        data
      );
      const fontSizes = new Binding("string", this._fontSizeBinding).map(data);
      const fontWeights = new Binding("string", this._fontWeightBinding).map(
        data
      );
      const fontStyles = new Binding("string", this._fontStyleBinding).map(
        data
      );

      const attempt = () => {
        this._textLayouts = texts.map((x, i) => {
          const font = new TextCache.Font(
            fontFamilys[i].toString(),
            fontSizes[i] as number,
            fontWeights[i].toString(),
            fontStyles[i].toString()
          );
          const info = this._textCache.addText(x.toString(), font);
          return {
            size: [info.w, info.h],
      ...
      this._textInfo.value((d, i) => [
        this._textLayouts[i].size[0],
        this._textLayouts[i].size[1],
        this._textLayouts[i].position[0],
        this._textLayouts[i].position[1]
      ]);
      this.attr("mTextInfo", this._textInfo);
