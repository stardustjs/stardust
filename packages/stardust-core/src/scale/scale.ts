...
export class ScaleBinding {
  ...
  public getAttributes(): ScaleBindingAttributeInfo[] {
    const result: ScaleBindingAttributeInfo[] = [];
    for (const attr of this._scale.getAttributes()) {
      result.push({
        scaleBinding: this,
  ...
  public getExpression(attrs: {
    [name: string]: Specification.Expression;
  }): Specification.Expression {
    const sAttrs: { [name: string]: Specification.Expression } = {};
    for (const attr of this._scale.getAttributes()) {
      sAttrs[attr.name] = attrs[`s${attr.name}`];
    }
    const values = this._args.map((arg, index) => {
      if (arg instanceof ScaleBinding) {
        const a = arg as ScaleBinding;
        const attributes = a.getAttributes();
        const aAttrs: { [name: string]: Specification.Expression } = {};
        for (const attr of attributes) {
...
export interface Scale {
  ...
  getExpression: (
    attrs: { [name: string]: Specification.Expression },
    ...values: Specification.Expression[]
  ) => Specification.Expression;
