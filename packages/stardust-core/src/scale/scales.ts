...
export function linear(valueType: string = "float"): DomainRangeScale {
  const scale = ((...args: ScaleArgument[]): ScaleBinding => {
    return new ScaleBinding(scale, valueType, ["float"], ...args);
  }) as any;

  const domain: [BindingValue, BindingValue] = [0, 1];
  const range: [BindingValue, BindingValue] = [0, 1];
...
export function vector2Scale() {
  ...
  scale.getExpression = (attrs, value1, value2) =>
    SC.func("Vector2", "Vector2", value1, value2);
  return scale;
}
...
export function custom(expr: string): CustomScale {
  ...
  const scale = ((...args: ScaleArgument[]): ScaleBinding => {
    ...
    const vars = new Dictionary<Specification.Expression>();
    ...
    const e = compileExpression(parsed, vars);
    return new ScaleBinding(scale, e.valueType, ["float"], ...args);
  }) as any;

  const attributes = new Dictionary<{ type: string; value: BindingValue }>();

  scale.attr = (
    name: string,
    typeOrValue: string | BindingValue,
    value?: BindingValue
  ) => {
    if (typeOrValue == null && value == null) {
      return attributes.get(name).value;
    } else if (typeof typeOrValue == "string") {
      attributes.set(name, { type: typeOrValue, value });
      return scale;
    } else {
      attributes.set(name, {
        type: "float",
        value: typeOrValue as BindingValue
      });
      return scale;
    }
  };
  ...
  scale.getAttributes = () => {
    const r: ScaleAttributeInfo[] = [];
    attributes.forEach((attr, name) => {
      r.push({ name, type: attr.type, binding: attr.value });
    });
    return r;
  };
  scale.getExpression = (attrs: any, value: any) => {
