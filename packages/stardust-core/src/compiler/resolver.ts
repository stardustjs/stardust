...
export class ModuleResolver {
  ...
  private _currentMoudles: Array<Dictionary<SyntaxTree.FileBlockFunction>>;

  constructor() {
    this._functions = new Dictionary<FunctionOverloadResolver>();
    this._functionModule = new Dictionary<
      Dictionary<SyntaxTree.FileBlockFunction>
    >();
    ...
    Intrinsics.forEachIntrinsicFunction(info => {
      this.addIntrinsicFunction(info.name, {
        ...
        arguments: info.argTypes.map((x, idx) => ({
          name: "a" + idx,
          type: x
        })),
        statements: null
  ...
  public addIntrinsicFunction(
    name: string,
    func: SyntaxTree.FileBlockFunction
  ) {
    if (!this._functions.has(name)) {
      const resolver = new FunctionOverloadResolver(name);
      this._functions.set(name, resolver);
      resolver.addFunction(func);
    } else {
      const resolver = this._functions.get(name);
      resolver.addFunction(func);
    }
  }
  ...
  public importFunction(
    module: Dictionary<SyntaxTree.FileBlockFunction>,
    name: string
  ) {
  ...
  public getFunction(name: string): FunctionOverloadResolver {
    for (let i = this._currentMoudles.length - 1; i >= 0; i--) {
      const cm = this._currentMoudles[i];
      if (cm && cm.has(name)) {
        const resolver = new FunctionOverloadResolver(name);
...
export class FunctionOverloadResolver {
  ...
  public resolveArguments(
    ...
    for (const func of this._functions) {
      let funcRank = 0;
      let matched = true;
      const argExpressions: Specification.Expression[] = [];
      const argIndexUsed: string[] = [];
      const kwargsUsed: string[] = [];
      for (const argIndex in func.arguments) {
        const arg = func.arguments[argIndex];
        const argExpression = args[argIndex] || kwargs[arg.name];
        ...
        if (argExpression != null) {
          if (argExpression.valueType != arg.type) {
            const [conversion, rank] = typeConversionAttempt(
              argExpression,
              arg.type
            );
            if (conversion) {
    ...
    if (result) {
      ...
      const argspec = args.map(x => x.valueType).join(", ");
      throw new CompileError(
        `unable to resolve function '${this._name}' with arguments (${argspec})`
      );
    }
  }
}

export function typeConversionAttempt(
  src: Specification.Expression,
  dest: string
): [Specification.Expression, number] {
  const info = Intrinsics.getTypeConversion(src.valueType, dest);
  if (info) {
