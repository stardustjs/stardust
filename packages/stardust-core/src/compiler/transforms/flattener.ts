...
const SC = Specification.Construct;
...
export function flattenEmits(mark: Specification.Mark): FlattenedEmits {
  const vertexIndexNameFloat = attemptName(
    "s3idx",
    c => !mark.variables.hasOwnProperty(c) && !mark.input.hasOwnProperty(c)
  );
  const vertexIndexName = attemptName(
    "s3idx_i",
    c => !mark.variables.hasOwnProperty(c) && !mark.input.hasOwnProperty(c)
  );
  const emitIndexName = attemptName(
    "s3emitidx",
    c => !mark.variables.hasOwnProperty(c) && !mark.input.hasOwnProperty(c)
  );
  const newMark: Specification.Mark = {
    input: {},
    output: mark.output,
  ...
  const generateStatements = (
  ...
  ): [Specification.Statement[], number] => {
    const result: Specification.Statement[] = [];
    ...
    for (let i = 0; i < statements.length; i++) {
      switch (statements[i].type) {
        ...
        case "for":
          {
            const forStatement = statements[
              i
            ] as Specification.StatementForLoop;
            const [generatedStatements, maxNumber] = generateStatements(
              ...
              tStatements.push({
                ...
                expression: SC.add(
                  ...
                  SC.mul(
                    SC.constant(maxNumber, "int"),
                    SC.sub(
                      SC.variable(forStatement.variableName, "int"),
                      SC.constant(forStatement.rangeMin, "int")
                    )
                  )
                )
              ...
              tStatements.push({
                ...
                expression: SC.add(
                  ...
                  SC.mul(
                    SC.constant(maxNumber, "int"),
                    SC.sub(
                      SC.constant(forStatement.rangeMax, "int"),
                      SC.variable(forStatement.variableName, "int")
                    )
                  )
                )
                ...
                condition: SC.op(
                  ...
                  SC.lessThan(
                    ...
                    SC.add(
                      ...
                      SC.constant(
                        maxNumber *
                          (forStatement.rangeMax - forStatement.rangeMin + 1),
                        "int"
                      )
                    )
                  )
        ...
        case "condition":
          {
            const condStatement = statements[
              i
            ] as Specification.StatementCondition;
            const [gTrueStatements, gTrueStatementsMax] = generateStatements(
              condStatement.trueStatements
            );
            const [gFalseStatements, gFalseStatementsMax] = generateStatements(
              condStatement.falseStatements
            );
            result.push({
              type: "condition",
  ...
  const [generatedStatements, maxNumberEmits] = generateStatements(
    mark.statements
  );
