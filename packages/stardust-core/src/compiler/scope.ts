...
export class ScopeVariables {
  ...
  constructor(
    owner: ScopeStack,
    parentScope: ScopeVariables = null,
    argMap: Dictionary<string> = null
  ) {
    this._owner = owner;
  ...
  public addVariable(name: string, type: string) {
    if (
      ...
      const translatedName = this._owner.newTranslatedName(name, type);
...
export class ScopeStack {
  ...
  public newTranslatedName(name: string, type: string): string {
    const candidate = attemptName(name, c => !this._translatedNames.has(c));
