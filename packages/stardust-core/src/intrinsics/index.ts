...
const intrinsicFunctions = new Dictionary<IntrinsicFunctionInfo>();
const intrinsicFunctionList: IntrinsicFunctionInfo[] = [];
const typeConversions = new Dictionary<TypeConversionInfo>();
const constants = new Dictionary<ConstantInfo>();

export function getInternalName(func: {
  name: string;
  argTypes: string[];
  returnType: string;
}) {
...
export function getIntrinsicFunction(
  internalName: string
): IntrinsicFunctionInfo {
...
export function forEachIntrinsicFunction(
  callback: (info: IntrinsicFunctionInfo) => any
) {
...
export function forEachTypeConversion(
  callback: (info: TypeConversionInfo) => any
) {
  typeConversions.forEach(callback);
}

export function getTypeConversion(
  srcType: string,
  destType: string
): TypeConversionInfo {
...
function RegisterTypeConversion(
  srcType: string,
  destType: string,
  rank: number,
  func: IntrinsicFunction
) {
  const name = `cast:${srcType}:${destType}`;
  const info: IntrinsicFunctionInfo = {
    name,
    argTypes: [srcType],
    returnType: destType,
...
const RegisterConstructor = (
  ...
  srcTypes: string[],
  func: IntrinsicFunction
) => RegisterFunction(type, srcTypes, type, func);
...
RegisterOperator(
  ...
  ["Vector3", "Vector3"],
  "Vector3",
  (a: number[], b: number[]) => [a[0] + b[0], a[1] + b[1], a[2] + b[2]]
);
RegisterOperator(
  "-",
  ["Vector3", "Vector3"],
...
RegisterOperator("-", ["Vector3"], "Vector3", (a: number[]) => [
  -a[0],
  -a[1],
  -a[2]
]);
...
RegisterOperator("-", ["Vector4"], "Vector4", (a: number[]) => [
  -a[0],
  -a[1],
  -a[2],
  -a[3]
...
RegisterOperator(
  ...
  ["float", "Vector2"],
  "Vector2",
  (a: number, b: number[]) => [a * b[0], a * b[1]]
);
RegisterOperator(
  "*",
  ["Vector2", "float"],
  "Vector2",
  (a: number[], b: number) => [a[0] * b, a[1] * b]
);
RegisterOperator(
  "*",
  ["Vector2", "float"],
...
RegisterOperator(
  ...
  ["float", "Vector3"],
  "Vector3",
  (a: number, b: number[]) => [a * b[0], a * b[1], a * b[2]]
);
RegisterOperator(
  "*",
  ["Vector3", "float"],
  "Vector3",
  (a: number[], b: number) => [a[0] * b, a[1] * b, a[2] * b]
);
RegisterOperator(
  "*",
  ["Vector3", "float"],
...
RegisterOperator(
  ...
  ["float", "Vector4"],
  "Vector4",
  (a: number, b: number[]) => [a * b[0], a * b[1], a * b[2], a * b[3]]
);
RegisterOperator(
  "*",
  ["Vector4", "float"],
  "Vector4",
  (a: number[], b: number) => [a[0] * b, a[1] * b, a[2] * b, a[3] * b]
);
RegisterOperator(
  "*",
  ["Vector4", "float"],
...
RegisterOperator("==", ["float", "float"], "bool", (a: number, b: number) =>
  a == b ? 1 : 0
);
RegisterOperator(">", ["float", "float"], "bool", (a: number, b: number) =>
  a > b ? 1 : 0
...
RegisterOperator("==", ["int", "int"], "bool", (a: number, b: number) =>
  a == b ? 1 : 0
);
RegisterOperator(">", ["int", "int"], "bool", (a: number, b: number) =>
  a > b ? 1 : 0
...
RegisterOperator("==", ["bool", "bool"], "bool", (a: number, b: number) =>
  a == b ? 1 : 0
);
...
RegisterConstructor("Vector2", ["float", "float"], (a: number, b: number) => [
  a,
  b
]);
RegisterConstructor(
...
RegisterFunction("atan2", ["float", "float"], "float", (a: number, b: number) =>
  Math.atan2(a, b)
);

RegisterFunction("abs", ["int"], "int", (a: number) => Math.abs(a));
RegisterFunction("min", ["int", "int"], "int", (a: number, b: number) =>
  Math.min(a, b)
);
RegisterFunction("max", ["int", "int"], "int", (a: number, b: number) =>
  Math.max(a, b)
);
RegisterFunction("min", ["float", "float"], "float", (a: number, b: number) =>
  Math.min(a, b)
);
RegisterFunction("max", ["float", "float"], "float", (a: number, b: number) =>
  Math.max(a, b)
);
RegisterFunction("ceil", ["float"], "float", (a: number, b: number) =>
  Math.ceil(a)
);
RegisterFunction("floor", ["float"], "float", (a: number, b: number) =>
  Math.floor(a)
);
...
RegisterFunction("clamp", ["float"], "float", (a: number) =>
...
RegisterFunction("length", ["Vector2"], "float", (a: number[]) =>
  Vector2.FromArray(a)
    .normalize()
    .toArray()
);
RegisterFunction("length", ["Vector3"], "float", (a: number[]) =>
  Vector3.FromArray(a)
    .normalize()
    .toArray()
);
RegisterFunction("length", ["Vector4"], "float", (a: number[]) =>
  Quaternion.FromArray(a)
    .normalize()
    .toArray()
);
RegisterFunction("length", ["Quaternion"], "float", (a: number[]) =>
  Quaternion.FromArray(a)
    .normalize()
    .toArray()
);

RegisterFunction("normalize", ["Vector2"], "Vector2", (a: number[]) =>
  Vector2.FromArray(a).length()
);
RegisterFunction("normalize", ["Vector3"], "Vector3", (a: number[]) =>
  Vector3.FromArray(a).length()
);
RegisterFunction("normalize", ["Vector4"], "Vector4", (a: number[]) =>
  Quaternion.FromArray(a).length()
);
RegisterFunction("normalize", ["Quaternion"], "Quaternion", (a: number[]) =>
  Quaternion.FromArray(a).length()
);
...
RegisterFunction(
  ...
  "Vector3",
  (a: number[], b: number[]) => {
    return Vector3.FromArray(a)
      .cross(Vector3.FromArray(b))
      .toArray();
  }
);
...
RegisterFunction(
  ...
  "Quaternion",
  (a: number[], b: number[]) => {
    return Quaternion.FromArray(a)
      .mul(Quaternion.FromArray(b))
      .toArray();
  }
);
...
RegisterConstructor(
  "Color",
  ["float", "float", "float"],
  (r: number, g: number, b: number) => [r, g, b, 1]
);
RegisterConstructor("Color", ["float", "float"], (v: number, a: number) => [
  v,
  v,
  v,
  a
